...
@app.get("/{collection_name}/shard-count")
async def shard_count(collection_name: str):
    """Получить количество документов в каждом шарде"""
    try:
        # Получаем информацию о шардах
        shards_info = await client.admin.command("listShards")
        shard_counts = {}
        ...
        for shard in shards_info.get("shards", []):
            ...
            # Подключаемся к каждому шарду для подсчета документов
            try:
                # Извлекаем primary хост из строки подключения
                primary_host = shard_host.split('/')[1].split(',')[0]
                ...
                shard_counts[shard_name] = f"Error: {str(e)}"
...
@app.get("/{collection_name}/replica-count")
async def replica_count(collection_name: str):
    """Получить количество реплик в каждом шарде"""
    try:
        # Получаем информацию о шардах
        shards_info = await client.admin.command("listShards")
        replica_info = {}
        ...
        for shard in shards_info.get("shards", []):
            ...
            # Подсчитываем количество реплик в строке подключения
            replica_count = len(shard_host.split('/')[1].split(','))
            
            replica_info[shard_name] = {
                "replica_count": replica_count,
                "hosts": shard_host.split('/')[1].split(',')
            }
        
        # Получаем информацию о config server
        try:
            config_status = await client.admin.command("replSetGetStatus")
            config_replica_count = len(config_status.get("members", []))
            replica_info["config"] = {
                "replica_count": config_replica_count,
                "hosts": [member["name"] for member in config_status.get("members", [])]
            }
        except Exception as e:
            replica_info["config"] = {
