...
async def list_users(collection_name: str):
    ...
    time.sleep(1)
    collection = db.get_collection(collection_name)
    # Фильтруем только документы с полем 'age' (пользователи)
    users = await collection.find({"age": {"$exists": True}}).to_list(1000)
...
async def show_user(collection_name: str, name: str):
    ...
    collection = db.get_collection(collection_name)
    # Фильтруем только документы с полем 'age' (пользователи)
    if (user := await collection.find_one({"name": name, "age": {"$exists": True}})) is not None:
        return user
